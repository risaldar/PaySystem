...
public class DatabaseManager {
    ...
    public boolean testConnection() {
        ...
        try {
            System.out.println("\n------------------------\n - testConnection - 1\n");
            System.out.println(properties.get(JDBC_DRIVER));
            System.out.println(properties.get(JDBC_USER));
            System.out.println(properties.get(JDBC_PASSWORD));
            Class.forName(properties.get(JDBC_DRIVER));
            System.out.println("\n------------------------\n - testConnection - 2\n");
            System.out.println(properties.get(JDBC_URL));
            Connection conn = DriverManager.getConnection(properties.get(JDBC_URL), properties.get(JDBC_USER), properties.get(JDBC_PASSWORD));
            System.out.println("\n------------------------\n - testConnection - 3\n");
            conn.close(); // Database exists.
            return true;
        } catch (Exception e) {
            System.out.println("\n------------------------\n - testConnection catch (Exception e) \n");
            e.printStackTrace();
            return false;
        }
    }
    ...
    private HashMap<String, String> getProperties(boolean create) {
        ...
        switch (DBType.valueOf(PaySystemProperties.getProperty(PropertyName.DB_TYPE))) {
            ...
            case MySQL:
                ...
                properties.put(JDBC_DRIVER, "com.mysql.cj.jdbc.Driver");
                break;
        }
        return properties;
    }
